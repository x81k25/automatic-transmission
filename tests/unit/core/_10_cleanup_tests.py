    ...
    def test_cleanup_transferred_media(self, mock_get_media, mock_remove_item, mock_db_update,
        ...
        for case in cleanup_transferred_media_cases:
            ...
            if case["input_media"] is None:
                ...
                # Create a pl.DataFrame with the raw data containing updated_at
                df = pl.DataFrame(case["input_media"])
                # Add original_title if missing (required by schema validation)
                if 'original_title' not in df.columns:
                    df = df.with_columns(
                        pl.col('media_title').alias('original_title')
                    )
                mock_get_media.return_value = df
            ...
            # (detailed verification is complex due to mocking)
    ...
    def test_cleanup_hung_items(self, mock_current_items, mock_get_media_by_hash,
        ...
        for case in cleanup_hung_items_cases:
            ...
            if case["input_media"] is None:
                ...
                mock_get_media_by_hash.return_value = df
