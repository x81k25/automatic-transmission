...
from src.data_models import MediaSchema, RejectionStatus, MediaType
...
def process_new_items(new_transmission_items: dict) -> pl.DataFrame | None:
    """
    process new media items and return as formatted DataFrame; if the
    ...
    :param new_transmission_items: items with new hashes currently in
        transmission
    :return: DataFrame with all necessary elements
    """
    new_media = pl.DataFrame({
        ...
        'rejection_status': RejectionStatus.OVERRIDE.value
    }).with_columns(
        media_type=pl.col("original_title")
            .map_elements(utils.classify_media_type, return_dtype=pl.Utf8)
    ).with_columns(
        error_condition = pl.when(pl.col('media_type') == MediaType.UNKNOWN.value)
...
def update_rejected_status(rejected_media: pl.DataFrame) -> pl.DataFrame:
    """
    sets all rejections statuses to "override"

    :param rejected_media: DataFrame with any previous rejection status
    :return: updated DataFrame with correction rejection status
    """
    return rejected_media.with_columns(
        rejection_status = pl.lit(RejectionStatus.OVERRIDE.value)
    )
...
def collect_media():
    ...
    if len(new_hashes) > 0:
        ...
        if new_media.height > 0:
            new_media = MediaSchema.validate(new_media)
    ...
    if len(rejected_hashes) > 0:
        ...
        if rejected_media is not None:
            rejected_media = update_rejected_status(rejected_media)
            rejected_media = MediaSchema.validate(rejected_media)
            utils.media_db_update(media=rejected_media)
