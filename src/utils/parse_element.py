...
def extract_hash_from_direct_download_url(href: str) -> str | None:
    """
    Extract hash from direct download URL.

    :param href: URL string to extract hash from
    :return: Lowercase hash string or None if empty result
    """
    if not href:
        return None

    hash_result = href.split('/')[-1].lower()
...
def extract_hash_from_magnet_link(href:str) -> str | None:
    ...
    hash_result = href.split('urn:btih:')[1].split('&')[0].lower()
